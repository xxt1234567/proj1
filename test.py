...
for i in data:
    ...
    for j in range(len(data[i]['segmentation'])):
        ...
        #cv2.putText(im2,str(data[i]['category_id'][j]),((data[i]['segmentation'][j][0][0]+data[i]['segmentation'][j][1][0]+data[i]['segmentation'][j][2][0]+data[i]['segmentation'][j][3][0])//4,(data[i]['segmentation'][j][0][1]+data[i]['segmentation'][j][1][1]+data[i]['segmentation'][j][2][1]+data[i]['segmentation'][j][3][1])//4),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,0),2)
